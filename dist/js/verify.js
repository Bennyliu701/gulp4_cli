...
if (!mql.matches) {
  ...
  if ($('.scroll-body').height() > $('.common-body').height()) {
    if ($.cookie('curSize') && $('.common-body').height() != $.cookie('curSize').split('-')[0]) {
      $('.scroll-body').css({
        'height': $.cookie('curSize').split('-')[0]
      });
    } else {
      $('.scroll-body').css({
        'height': $('.common-body').height()
      });
    }
  }
...
var innerWidthTmp = window.innerWidth; //横竖屏事件监听方法
...
function screenListener() {
  try {
    ...
    if (iw != innerWidthTmp) {
      if (iw > window.innerHeight) orientation = 90;else orientation = 0; //调用转屏事件
...
var orientation = 0; //转屏事件，内部功能可以自定义
...
$(function () {
  ...
  var userType = $.cookie('userType') ? JSON.parse($.cookie('userType')) : ''; //sdk传入的参数
  ...
  if ($('.personal-center').length > 0) {
    ...
    if (!userData) {
      ...
      if (!userData || !userType) return; // userData.avatar?$('#hAvatar').attr('src', userData.avatar):$('#hAvatar').attr('src', 'img/user.png')
  ...
  $('#hSignupBtn').click(function () {
    ...
    if (Profile.accountCheck(user, userErr) && Profile.pwdCheck(pwd, pwdErr) && Profile.repwdCheck(pwd, repwd, repwdErr)) {
  ...
  $('.pitch-on').click(function () {
    ...
    if (isActive) {
      ...
      if (name == 'sms' || name == 'email') {
        ...
        $(_this).parent().addClass('user-selected'); //同意注销协议
  ...
  $('.common-form .sendMail').click(function (e) {
    ...
    switch (name) {
      case 'phone':
        ...
        if (Profile.phoneCheck(phone, phoneErr) && Profile.picCodeCheck(code, codeErr)) {
          Profile.sendPhoneCode(2, phone, code, function (data) {
            if (data.status === 1) {
              ...
              _this.attr('data-sid', data.data.session_id); // var data = {
              //     msg:'验证码已发送至邮箱'
              // };
              // popTip.initPop('1',data,function () {
              // })
      ...
      case 'unbindPhone':
        ...
        if (Profile.phoneCheck(unbindPhone, unbindPhoneErr) && Profile.picCodeCheck(unbindCode, unbindCodeErr)) {
  ...
  /* 提交认证表单 || 提交找回密码 || 提交找回账户 || 解绑账户 || 解绑邮箱 */
  ...
  $('.common-form .identify-btn').click(function () {
    ...
    switch (name) {
      case 'phone':
        // 手机提交认证
        if (type != '') {
          if (Profile.phoneCheck(phone, phoneErr) && Profile.txtCodeCheck(phoneCode, phoneCodeErr)) {
      ...
      case 'mail':
        // 邮箱提交认证
        if (Profile.mailCheck(mail, mailErr) && Profile.txtCodeCheck(mailCode, mailCodeErr)) {
          Profile.iMark = true;
        ...
        if (Profile.phoneCheck(phone, phoneErr) && Profile.txtCodeCheck(phoneCode, phoneCodeErr)) {
          ...
          Profile.getPwdPhone(phone, phoneCode, session_id, function (data) {
            ...
            if (data.status === 1) {
              ...
              popTip.initPop('0', data, function () {}); //phoneCodeErr.show();
      ...
      case 'unbindPhone':
        // 解绑手机
        if (Profile.phoneCheck(unbindPhone, unbindPhoneErr) && Profile.txtCodeCheck(unbindPhoneCode, unbindPhoneCodeErr)) {
      ...
      case 'unbindMail':
        // 解绑邮箱
        if (Profile.mailCheck(unbindMail, unbindMailErr) && Profile.txtCodeCheck(unbindMailCode, unbindMailCodeErr)) {
          Profile.iMark = true;
  ...
  if ($('.center-body').length > 0) {
    ...
    Profile.getMsgInfo(function (data) {
      if (data.status === 1) {
        ...
        var data = {
          msg: '서버 연결 실패 하였습니다. 잠시 후 다시 시도해주세요.' //msg:'시스템이 복잡하니 나중에 다시 시도해 주시기 바랍니다.'
  ...
  if ($('.account-body').length > 0) {
    ...
    Profile.getUserType(function (data) {
      if (data.status === 1) {
        ...
        // 有韩国又有gta账号-绑定第三方
        //onlyOne = true;
        var removerC = function removerC(name, classn) {
          if ($(name).hasClass(classn)) $('#hFb').removeClass(classn);
        };

        var data = data.data; //有官方账号首页显示修改密码，没有显示设置密码

        var userType = data;
        var fb = userType.facebook ? '연동 해제' : '연동',
            //'解绑':'绑定',
        gmail = userType.google ? '연동 해제' : '연동',
            //'解绑':'绑定',
        korea = userType.korea ? '연동 해제' : '연동',
            //'解绑':'绑定',
        fb1 = userType.facebook ? '연동됨' : '미연동 상태',
            //'已绑定':'未绑定',
        ...
        if (userType.korea || userType.gta) {
          $('.three-account').show();
          $('#hFb span').html(fb1);
          $('#hGmail span').html(gmail1); // $('#hKorea span').html(korea1);

          $('#hFb a').html(fb);
          $('#hGmail a').html(gmail); // $('#hKorea a').html(korea);
        ...
        $('.sure-btn-click').click(function () {
          var name = $(this).attr('name');
          $('.pop-account,.pop-mark').fadeOut(300); // goBind(name);
        ...
        $('.has-bind a').click(function (e) {
          e.preventDefault();
          if (!unbind) return;
          unbind = 0; //判断是否有官方账号，有官方账号显示需要绑定列表
  ...
  $('.logout-account').click(function () {
    ...
    if (isOnOff) {
      ...
      var params = $.cookie('params') ? JSON.parse($.cookie('params')) : '';
      var pwd = $('#aPwd').val() || '';
      Profile.getAjax('POST', '/site/rpcsdklogin', params, 'application/x-www-form-urlencoded', function (data) {
        if (data.status == 1) {
